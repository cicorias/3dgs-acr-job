...
# 1. Builds the container image via ACR Tasks (no local Docker required)
...
echo "╔══════════════════════════════════════════════╗"
echo "║  Post-Provision: ACR Image Build             ║"
echo "╚══════════════════════════════════════════════╝"
echo ""
...
if [[ -n "$JOB_IMAGE" && -n "$JOB_NAME" && -n "$RESOURCE_GROUP" ]]; then
  ...
  echo "   Run 'azd deploy' to apply the image to the job."
fi
